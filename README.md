...
1.	在java中，只要一个类实现了java.io.Serializable接口，那么它就可以被序列化
	...
	.	通过ObjectOutputStream和ObjectInputStream对对象进行序列化合反序列化操作

	.	 对象是否允许被反序列化，不仅仅是取决于对象的代码是否一致，同时还有一个重要的因素（UID）
...
以下是与微服务相关的一些最佳实践，您可以在基于微服务风格的体系结构进行应用程序实现时考虑这些实践：

**基于域驱动设计（DDD）的模型服务**：服务应该围绕业务领域建模。
...
**考虑分离数据存储**：数据应该对每个微服务都是私有的。微服务成为其数据的  所有者。 对 特定服务拥有的数据的任何访问都应该  只通过API进行。如果不这样做，将允许多个服务访问特定服务所拥有的数据库，从而导致  服务之间的耦合。诸如CQRS（命令和查询责任隔离）之类的体系结构模式在处理需要由不同类型的用户读取的数据时非常方便。
...
**设计域驱动的API**：API的设计应牢记业务领域。此外，实现细节不应成为API设计的一部分。

**设计内聚服务**：考虑将需要一起更改的功能分组为一个单元而不是单独的服务。不这样做会导致大量的服务间通信代表硬耦合。

考虑将服务分开以解决跨领域问题：应考虑为跨领域问题（如身份验证和授权）设计单独的服务。

**自动化进行独立部署**：设计精良的微服务应该能够独立部署。而且，构建和发布自动化将增强部署过程，从而实现更快的发布和更短的总体交付时间。这将有助于构建真正具有云原生性的微服务，其中微服务包装在容器中，并以简单的方式部署到包括云在内的任何环境中。 整个组织范围内的优秀  DevOps实践将有助于实现这一目标。

**故障隔离**：基于微服务的体系结构应考虑采用独立的微服务来实现故障隔离。架构原则和设计模式，如下面的一些将有助于实现相同：
...
API访问控制和网关：如何控制微服务的安全性。


应用程序编程接口（API）：一种软件接口，允许用户配置其他程序并与之交互，通常是通过从函数列表中调用。

## C

容器： OS（而不是机器）级别的资源隔离，通常（在基于UNIX的系统中）在用户空间中。隔离元素因容器化策略而异，通常包括文件系统，磁盘配额，CPU和内存，I / O速率，root权限和网络访问。比机器级虚拟化轻得多，足以满足许多隔离要求集。


持续交付：一种软件工程方法，其中持续集成，自动化测试和自动部署功能允许以最少的人为干预快速，可靠和重复地开发和部署软件。

## D

解耦：将单片应用程序分解为更小的组件，以便传统企业可以进行数字转换。


分布式系统：在广泛的服务或节点网络上运行的任何系统或应用程序。

分布式跟踪：一种工具和实践，允许开发人员通过创建记录有关执行的请求和操作的信息的服务来分析服务的行为并解决问题。

领域驱动设计：开发软件的理念，其中开发主要关注业务逻辑，应用程序应执行或解决的活动和问题。

## E

企业服务总线（ESB）：将消息传递系统与中间件相结合的实用程序，为软件应用程序提供全面的通信服务。

最终一致性：一种数据一致性模型，用于通过使数据在所有服务或节点之间保持同步和最新，使分布式应用程序具有高可用性。

## F

故障隔离：使微服务崩溃并重新启动，而不会导致服务中断。

Function as a service：一类云计算服务，提供允许客户开发，运行和管理应用程序功能的平台，而无需构建和维护通常与开发和启动应用程序相关联的基础结构的复杂性。

## H

Holacracy：一种管理实践，适用于根据角色分为自治和独立部门的组织，可以根据自己的职责组织自己并做出决策。Holacracies专注于快速迭代。

## J

Java虚拟机（JVM）：允许计算机运行Java程序的抽象软件。

## M

Message Broker：中间件，用于转换由一个软件发送的消息，以供另一个软件读取。

微服务架构：将应用程序设计为模块化服务的开发方法，可无缝适应高度可扩展的动态环境。

Monolith：一种软件架构，其中数据和UI都可以在一个平台上的单个程序中找到。

## O

业务流程：自动化应用程序和容器的管理和部署的方法。

OWAT2： OAuth的微服务安全最佳实践。

## S

无服务器：云计算执行模型，云提供商动态管理机器资源的分配。

服务发现：查找服务实例的网络位置以供进一步使用的行为。

Service Mesh：专注于服务到服务通信的基础架构层，主要用于分布式系统和云原生应用程序。

Sociocracy：一种没有集中权力结构的治理模式，旨在降低团队之间的独立性，专注于组织范围的战略。

## T

S十二因素应用技术：构建现代，可扩展，可维护的软件即服务应用程序的方法。

## W

SWeb服务：可以使用通过HTTP访问并在远程系统上执行的API以标准化方式通过Web访问的功能。
