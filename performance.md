...
3. Java 8中使用了自动并行化的特性会用到-个公共的ForkJoinPool实例。我们可能需要根据实际情况调整这个实例的默认大小。
...
1. 通过合理配置JDBC或者JPA,尽可能地实现批量读取和写人。
...
1.处理异常的代价未必会很高，不过还是应该在适合的时候才用。2.栈越深，处理异常的代价越高。
...
2.对于多行的连接操作，-定要确保使用StringBuilder。
